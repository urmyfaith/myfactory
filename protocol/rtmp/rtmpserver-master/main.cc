		...
		const AMFValue &status = AMFValue())
{
	if (txid < 0.0)
	{
		printf("send_reply txid error %lf \n",txid);
		return;
	}
...
int recv_from_client(Client *client)
{
//	printf("recv_from_client begin\n");
...
/*		printf("RTMP_Header = %u %u %u %u %u %u %u\n", 
			header.timestamp[0], header.timestamp[1], header.timestamp[2],
			(uint8_t)header.msg_len[0], (uint8_t)header.msg_len[1], (uint8_t)header.msg_len[2],
			header.msg_type);
*/
	...
	for (size_t i = 0; i < poll_table.size(); ++i) {
		...
		if (poll_table[i].revents & POLLIN) {
			if (client == NULL) {
				...
				try {
					int ret = recv_from_client(client);
					if( ret < 0 )
					{
						close_client(client, i);
						--i;						
